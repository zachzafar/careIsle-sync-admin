...
export default function ApiKeysPage() {
  const [facilityUniqueId, setFacilityUniqueId] = useState("")
  ...
  const { data: facilityResp, isPending, error } = tsr.facilities.getOne.useQuery({
    queryKey: ["facility", facilityUniqueId],
    queryData:{
      params: { id: facilityUniqueId },
    }
  })
  ...
  useEffect(() => {
    ...
    if (status && status !== 401) {
      toast.error("Failed to load keys", { description: "Check the facility ID and try again." })
    }
  }, [error])

  const apiKeys = useMemo(() => {
    const facility = facilityResp?.status === 200 ? facilityResp.body : undefined
    ...
    onSuccess: (data) => {
      if (data.status === 201) {
        ...
        toast.success("API key created", {
          description: `Key "${created.name}" created`,
        })
        // Refresh the facility keys list
        tsrQueryClient.invalidateQueries({ queryKey: ["facility", facilityUniqueId] })
      } else {
  ...
  // For delete confirmation modal state
  const [pendingDelete, setPendingDelete] = useState<string | null>(null)
  const onConfirmDelete = () => {
    if (!pendingDelete) return
    deleteMutation.mutate({
      params: { id: pendingDelete }, // id is the actual key string
      ...
      <div className="max-w-3xl space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Create, view, copy, and delete API keys scoped to a facility.
          </p>
        </div>
              ...
              {!facilityUniqueId.trim() ? (
                <TableRow>
                  <TableCell colSpan={3} className="text-center py-8 text-muted-foreground">
                    Enter a facility ID to view keys.
                  </TableCell>
                </TableRow>
              ) : isPending ? (
                <TableRow>
